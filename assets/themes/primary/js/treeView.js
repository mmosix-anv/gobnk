(function ($) {
    ...
    function treeView($me) {
        ...
        // collapsable elements i.e. the li with an ul in it
        ...
        function generateTree(data, $root, useSpan, imgList) {
            ...
            function createNode(nObj, $target) {
                ...
                if (imgList.length > 0) {
                    // the image
                    let image = 'url(' + imgList[nObj.imageIndex] + ')';
                ...
                if (nObj.children !== undefined && nObj.children.length > 0) {
                    let innerList = $('<ul>').appendTo(li);
                    for (let i = 0; i < nObj.children.length; i++) {
                        let child = nObj.children[i];
        ...
        return {
            ...
            // expand all items
            expandAll: function () {
                let items = $me.find('.contains-items');
