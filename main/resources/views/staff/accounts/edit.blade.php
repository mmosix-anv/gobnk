...
@section('master')
    <div class="col-12">
        <div class="custom--card">
            <form action="{{ route('staff.accounts.update', $user->account_number) }}" method="post" enctype="multipart/form-data">
                @csrf
                    ...
                    <div class="row g-lg-4 g-3">
                        <div class="col-lg-4 col-sm-6">
                            ...
                            <div class="upload__img">
                                ...
                                <input type="file" id="image" class="image-upload" name="image" accept=".jpeg, .jpg, .png">
                                <label for="image" class="upload__img-preview image-preview">
                                    <img src="{{ getImage(getFilePath('userProfile') . "/$user->image", getFileSize('userProfile'), true) }}" alt="@lang('User Image')">
                                </label>
                        ...
                        <div class="col-lg-8 col-sm-6">
                            <div class="row g-lg-4 g-3">
                                <div class="col-lg-6">
                                    ...
                                    <input type="text" id="first_name" class="form--control" name="firstname" value="{{ old('firstname', $user->firstname) }}" required>
                                </div>
                                <div class="col-lg-6">
                                    <label for="last_name" class="form--label required">@lang('Last Name')</label>
                                    <input type="text" id="last_name" class="form--control" name="lastname" value="{{ old('lastname', $user->lastname) }}" required>
                                </div>
                                <div class="col-lg-6">
                                    <label for="username" class="form--label required">@lang('Username')</label>
                                    <input type="text" id="username" class="form--control check-user" name="username" value="{{ old('username', $user->username) }}" required>
                                    <small></small>
                                </div>
                                <div class="col-lg-6">
                                    <label for="email" class="form--label required">@lang('Email Address')</label>
                                    <input type="email" id="email" class="form--control check-user" name="email" value="{{ old('email', $user->email) }}" required>
                                    <small></small>
                                </div>
                                <div class="col-lg-6">
                                    ...
                                    <select id="country" class="form--control form-select select-2" name="country_name" required>
                                        @foreach($countries as $key => $country)
                                            <option value="{{ $country->country }}" data-country_code="{{ $key }}" data-dial_code="{{ $country->dial_code }}" @selected(old('country_name', $user->country_name) == $country->country)>
                                                {{ $country->country }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-lg-6">
                                    ...
                                    <div class="input--group">
                                        ...
                                        <input type="text" id="mobile" class="form--control check-user" name="mobile" value="{{ old('mobile', $user->contact_number) }}" required>
                                    </div>
                                    <small></small>
                                </div>
                                <div class="col-12">
                                    <label for="address" class="form--label required">@lang('Address')</label>
                                    <input type="text" id="address" class="form--control" name="address" value="{{ old('address', $user->address->address) }}" required>
                                </div>
                                <div class="col-lg-4">
                                    <label for="city" class="form--label required">@lang('City')</label>
                                    <input type="text" id="city" class="form--control" name="city" value="{{ old('city', $user->address->city) }}" required>
                                </div>
                                <div class="col-lg-4">
                                    <label for="state" class="form--label">@lang('State')</label>
                                    <input type="text" id="state" class="form--control" name="state" value="{{ old('state', $user->address->state) }}">
                                </div>
                                <div class="col-lg-4">
                                    <label for="zip_code" class="form--label required">@lang('ZIP Code')</label>
                                    <input type="text" id="zip_code" class="form--control" name="zip_code" value="{{ old('zip_code', $user->address->zip) }}" required>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        (function ($) {
            ...
            function checkUniqueness(fieldName, fieldValue, feedbackElement) {
                $.get("{{ route('staff.accounts.check.unique', $user->id) }}", { field: fieldName, value: fieldValue }, function (response) {
                    if (response.exists) {
